...
const CarDetails = () => {
  ...
  useEffect(() => {
    ...
    setCar(foundCar || null); // set null if not found
  }, [cars, id]);

  if (car === undefined) {
    return <p className='text-center text-gray-500 mt-10'>Loading car details...</p>;
  }
        ...
        transition={{duration: 0.5,delay: 0.2}}
            className='grid grid-cols-2 sm:grid-cols-4 gap-4'> 
              ...
              <div>
                ...
                <ul className='grid grid-cols-l sm:grid-cols-2 gap-2' >
                  {
      ...
      className='shadow-lg h-max sticky top-18 rounded-xl p-6 space-y-6 text-gray-500'>
          ...
          <div className='flex flex-col gap-2' >
            ...
            <input type="date" value={pickupDate} onChange={(e)=>setPickupDate(e.target.value)}
             className='border border-borderColor rounded-xl px-3 py-2 rouned-lg' required id="pickup-date" min={new Date().toISOString().split('T')[0]}/>
          </div>
          <div className='flex flex-col gap-2' >
            <label htmlFor="return-date">Return Date</label>
            <input type="date" value={returnDate} onChange={(e)=>setReturnDate(e.target.value)} className='border border-borderColor rounded-xl px-3 py-2 rouned-lg' required id="return-date" min={new Date().toISOString().split('T')[0]}/>
          </div>
